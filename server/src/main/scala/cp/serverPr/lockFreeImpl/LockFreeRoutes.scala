...
object LockFreeRoutes {
  ...
  private val state = new LockFreeServerState()
  ...
  /**
   * Queue a process request using lock-free approach
   */
  private def queueProcessRequest(cmd: String, userIp: String): IO[String] = {
    IO.blocking {
      ...
      // Try to execute immediately using CAS operations
      if (state.tryStartExecution(requestId)) {
  ...
  /**
   * Execute a process with lock-free completion handling
   */
  ...
  /**
   * Process the next request in the queue using lock-free operations
   */
    ...
    state.getNextQueuedRequest() match {
      case Some((requestId, cmd, userIp)) =>
        ...
        val thread = new Thread(() => {
          val _ = executeProcess(cmd, userIp, requestId)  // Explicitly discard the result
        })
        thread.start()
      case None =>
